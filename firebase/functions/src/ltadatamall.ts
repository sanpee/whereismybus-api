 ...
 * @param {string} apiKey APIKey for the LTA Datamall API.
...
export async function findNearestBusStops(
  ...
  if (apiKey == "" || apiKey == null || apiKey == undefined) {
    ...
    return Promise.reject(new Error("Remember to set APIKey!"));
  }

  let allBusStops: BusStop[] = busStops;
  if (allBusStops==null) {
    allBusStops = await getAllBusStops(apiKey);
  }
...
export async function getBusArrivals(apiKey:string, busStopCode: string): Promise<BusServicesResult> {
  if (apiKey == "" || apiKey == null || apiKey == undefined) {
    console.log("Remember to set APIKey!");
    return Promise.reject(new Error("Remember to set APIKey!"));
  }
...
export async function getBusArrivalsv2(apiKey:string, busStopCode: string): Promise<BusServicesResult2> {
  if (apiKey == "" || apiKey == null || apiKey == undefined) {
    console.log("Remember to set APIKey!");
    return Promise.reject(new Error("Remember to set APIKey!"));
  }
...
export async function getBusServices(apiKey:string, busStopCode: string): Promise<string[]> {
...
export async function getAllBusStops(apiKey: string): Promise <BusStop[]> {
  if (apiKey == "" || apiKey == null || apiKey == undefined) {
    console.log("Remember to set APIKey!");
    return Promise.reject(new Error("Remember to set APIKey!"));
  }
...
export async function main() {
  const APIKey: string = process.env.LTADATAMALL_KEY;
  ...
  const allBusStops: Array<BusStop> = await getAllBusStops(APIKey);
  ...
  const busServices = await getBusServices(APIKey, "03059");
  console.log(busServices);

  const busArrivals = await getBusArrivals(APIKey, "14141");
  console.log(busArrivals);

  const busArrivals2 = await getBusArrivalsv2(APIKey, "14141");
